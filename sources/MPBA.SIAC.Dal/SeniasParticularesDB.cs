...
public static SeniasParticularesList GetListByidAutor(int idAutor)
{
    ...
    using (SqlConnection myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings[1].ConnectionString))
    {
        using (SqlCommand myCommand = new SqlCommand("SeniasParticularesSelectListByidAutor", myConnection))
        {
            myCommand.CommandType = CommandType.StoredProcedure;
...
public static int Save(SeniasParticulares mySeniasParticulares)
{
    ...
    using (SqlConnection myConnection = new SqlConnection(ConfigurationManager.ConnectionStrings[1].ConnectionString))
    {
        using (SqlCommand myCommand = new SqlCommand("SeniasParticularesInsertUpdateSingleItem", myConnection))
        {
            ...
            if (string.IsNullOrEmpty(mySeniasParticulares.descripcion))
            {
                myCommand.Parameters.AddWithValue("@descripcion", DBNull.Value);
            }
            else
            {
                myCommand.Parameters.AddWithValue("@descripcion", mySeniasParticulares.descripcion);
            }
            if (mySeniasParticulares.idAutor == null)
            {
                myCommand.Parameters.AddWithValue("@idAutor", DBNull.Value);
            }
            else
            {
                myCommand.Parameters.AddWithValue("@idAutor", mySeniasParticulares.idAutor);
            }
...
public static int Save(SeniasParticulares mySeniasParticulares, SqlCommand myCommand)
{
    ...
    try
    {
        ...
        if (mySeniasParticulares.descripcion == null)
        {
            myCommand.Parameters.AddWithValue("@Descripcion", DBNull.Value);
        }
        else
...
private static SeniasParticulares FillDataRecord(IDataRecord myDataRecord)
{
    ...
    {
        mySeniasParticulares.descripcion = myDataRecord.GetString(myDataRecord.GetOrdinal("descripcion"));
    }
    if (!myDataRecord.IsDBNull(myDataRecord.GetOrdinal("idAutor")))
    {
        mySeniasParticulares.idAutor = myDataRecord.GetInt32(myDataRecord.GetOrdinal("idAutor"));
    }
    return mySeniasParticulares;
}
}
