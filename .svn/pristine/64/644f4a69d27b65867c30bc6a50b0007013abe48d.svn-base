...
namespace Capturer.Forms
{
    public sealed partial class FingerSelector : Panel
	{
        ...
        private bool _manoAmputada = false;
		...
		public FingerSelector()
		{
			...
			_painter = new SvgPainter(Encoding.UTF8.GetString(FingerCapturer.Properties.Resources.TwoHands));
		}
		...
		#region Private methods
        public void SetManoAmputada(bool seteo)
        {
            _manoAmputada = seteo;
        ...
        private string IdentificaAmputacionMano(Point p)
        {
            if (p.X >= 4 && p.X <= 97 &&  p.Y <= 96  && p.Y >= 1)
            {  return "I";}

            if (p.X >= 115 && p.X <= 208 &&   p.Y >=1 && p.Y <= 95 )
            {  return "D";}
            return "N";
        }
		...
		private void FingerSelectorMouseClick(object sender, MouseEventArgs e)
		{
			if (FingerClick != null)
			{
                ...
                if  ( _manoAmputada && IdentificaAmputacionMano(p) == "I" )
                   {
                        click(this, new FingerClickArgs(NFPosition.LeftLittleFinger, e));
                        click(this, new FingerClickArgs(NFPosition.LeftMiddleFinger, e));
                        click(this, new FingerClickArgs(NFPosition.LeftRingFinger, e));
                        click(this, new FingerClickArgs(NFPosition.LeftThumb, e));
                        click(this, new FingerClickArgs(NFPosition.LeftIndexFinger, e));
                        return;
                    }

                if (_manoAmputada && IdentificaAmputacionMano(p) == "D")
                {
                    click(this, new FingerClickArgs(NFPosition.RightLittleFinger, e));
                    click(this, new FingerClickArgs(NFPosition.RightMiddleFinger, e));
                    click(this, new FingerClickArgs(NFPosition.RightRingFinger, e));
                    click(this, new FingerClickArgs(NFPosition.RightThumb, e));
                    click(this, new FingerClickArgs(NFPosition.RightIndexFinger, e));
                    return;
                }
